...
public class InfoSesion extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (session.isNew()) {
		  ...
		  session.getAttribute("primeraVez");
		} else {
		...
		// mensaje de bienvenida si es la primera vez que el usuario inicia sesión
		
		if (session.getAttribute("primeraVez")==null) {
			out.println("Bienvenid@. Esta es tu primera visita, se ha creado una sesión");
		} else {
			out.println("Has iniciado nuevamente sesión");
		}
