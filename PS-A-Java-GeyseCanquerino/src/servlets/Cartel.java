...
public class Cartel extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		boolean error_dia_ausente = false;

		if (error_dia_ausente) {
			out.println("<h3>Error: falta el dia de actuación</h3>");
		} else {
			...
			try {
				...
				String consulta = "SELECT * FROM actuacion, dias WHERE actuacion.dia=dias.id AND dias.dia=" + dia;
				ResultSet rset = sentencia.executeQuery(consulta);
				if (!rset.isBeforeFirst()) {
					out.println("<h3>No existe</h3>");
				} else {
					// Continuar
					out.println("<h3>ACTUACIONES DEL DIA"+dia+" </h3>");
					out.println("<ul>");
					while (rset.next()) {
						out.print("<ol>Nombre :<a href='Artista?nombre=&#39;"+rset.getString("nombre")+"&#39;'>" + rset.getString("nombre") + " </a></ol><br/>"
